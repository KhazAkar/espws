		...
		uint8_t regData)
{
	...
	i2c_master_writeByte(writeReg);
	if (!i2c_master_check_ack())
	{
#ifdef BME280_DEBUG
		ets_uart_printf("BME280_sendI2cWriteData: i2c_master_writeByte(%X) slave not ack..\r\n", writeReg);
#endif
		i2c_master_stop();
		return 0;
	}
	i2c_master_writeByte(regData);
	if (!i2c_master_check_ack())
	{
#ifdef BME280_DEBUG
		ets_uart_printf("BME280_sendI2cWriteData: i2c_master_writeByte(%X) slave not ack..\r\n", regData);
...
{
	i2c_master_start();
	i2c_master_writeByte(BME280_W);
	if (!i2c_master_check_ack())
	{
#ifdef BME280_DEBUG
		ets_uart_printf("BME280_startI2cWrite: i2c_master_writeByte(BME280_W) slave not ack..\r\n");
#endif
		i2c_master_stop();
		return 0;
	}
...
bool ICACHE_FLASH_ATTR BME280_sendI2cRead(uint8_t readReg)
{
	...
	{
#ifdef BME280_DEBUG
		ets_uart_printf("BME280_sendI2cRead: i2c_master_writeByte(BME280_W) slave not ack..\r\n");
#endif
		i2c_master_stop();
		return 0;
	}

	i2c_master_writeByte(readReg);
	if (!i2c_master_check_ack())
	{
#ifdef BME280_DEBUG
		ets_uart_printf("BME280_sendI2cRead: i2c_master_writeByte(readReg) slave not ack..\r\n");
#endif
		i2c_master_stop();
		return 0;
	}
	i2c_master_start();
	i2c_master_writeByte(BME280_R);
	if (!i2c_master_check_ack())
	{
#ifdef BME280_DEBUG
		ets_uart_printf("BME280_sendI2cRead: i2c_master_writeByte(BME280_R) slave not ack..\r\n");
#endif
		i2c_master_stop();
		return 1;
	}
...
bool ICACHE_FLASH_ATTR BME280_verifyChipId(void)
{
	...
	uint8_t version = i2c_master_readByte();
	i2c_master_send_ack(0);
	i2c_master_stop();
...
void ICACHE_FLASH_ATTR BME280_writeConfigRegisters(void)
{
	...
	uint8_t ctrl_meas_reg = (osrs_t << 5) | (osrs_p << 2)
...
signed long int BME280_calibration_Temp(signed long int adc_T)
{
	...
	var1 = ((((adc_T >> 3) - ((signed long int) calib_dig_T1 << 1)))
			* ((signed long int) calib_dig_T2)) >> 11;
	var2 = (((((adc_T >> 4) - ((signed long int) calib_dig_T1))
			* ((adc_T >> 4) - ((signed long int) calib_dig_T1))) >> 12)
			* ((signed long int) calib_dig_T3)) >> 14;
...
unsigned long int BME280_calibration_Press(signed long int adc_P)
{
	...
	var2 = (((var1 >> 2) * (var1 >> 2)) >> 11)
			* ((signed long int) calib_dig_P6);
	var2 = var2 + ((var1 * ((signed long int) calib_dig_P5)) << 1);
	var2 = (var2 >> 2) + (((signed long int) calib_dig_P4) << 16);
	var1 = (((calib_dig_P3 * (((var1 >> 2) * (var1 >> 2)) >> 13)) >> 3)
			+ ((((signed long int) calib_dig_P2) * var1) >> 1)) >> 18;
	...
	var1 = (((signed long int) calib_dig_P9)
			* ((signed long int) (((P >> 3) * (P >> 3)) >> 13))) >> 12;
	var2 = (((signed long int) (P >> 2)) * ((signed long int) calib_dig_P8))
			>> 13;
	P = (unsigned long int) ((signed long int) P
			+ ((var1 + var2 + calib_dig_P7) >> 4));
	return P;
}
...
unsigned long int BME280_calibration_Hum(signed long int adc_H)
{
	...
	v_x1 = (((((adc_H << 14) - (((signed long int) calib_dig_H4) << 20)
			- (((signed long int) calib_dig_H5) * v_x1))
			+ ((signed long int) 16384)) >> 15)
			* (((((((v_x1 * ((signed long int) calib_dig_H6)) >> 10)
					* (((v_x1 * ((signed long int) calib_dig_H3)) >> 11)
							+ ((signed long int) 32768))) >> 10)
					+ ((signed long int) 2097152))
					* ((signed long int) calib_dig_H2) + 8192) >> 14));
	v_x1 = (v_x1
			- (((((v_x1 >> 15) * (v_x1 >> 15)) >> 7)
					* ((signed long int) calib_dig_H1)) >> 4));
	v_x1 = (v_x1 < 0 ? 0 : v_x1);
